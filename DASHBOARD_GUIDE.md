...
| **Couple (Host)** | Celebration management, guest approvals, venue gate check-in | Active Weddings, Pending Requests, 78% Revenue, Gate Scanner | `/dashboard/celebrations` |
| **Agent** | Tracking referral clicks, conversions, commission payouts | Referral Link Generator, Click Metrics, Tiered Accrued Commissions (₹500-₹1800) | `/dashboard/referrals` |
...
## 3. Host (Couple) Dashboard (`/dashboard/celebrations`, `/dashboard/earnings`)
...
### Key Widgets & Components:
- **Active Weddings Overview**: Manages published Our Indian Weddings, capacity limits (`guestsAllowed`), and price per guest in INR (`WeddingCard.tsx`).
...
- **Host Financial Ledger**: Tracks gross guest booking volume, 78% host payout share in INR, and bank transfer statuses (`/dashboard/earnings`).
...
### Daily Operating Purpose
Freelance referral partners and luxury travel agents monitor click conversions, track referral link usage (`WWI-AGENT-XXXX`), and request payouts for accrued tiered commission (₹500-₹500) fees.
...
- **Commission Accounting Ledger**: Calculates accrued tiered referral commissions per booking with instant **Request Payout** triggers.
...
### Daily Operating Purpose
Platform operators oversee global marketplace health, audit host identity verifications, approve agent applications, review platform financial splits (22% platform fee), and triage safety cases.
