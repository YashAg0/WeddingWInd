...
| **Admin** | Internal Operations Managers | Operations Hub (`/dashboard/admin`) | Listing verification queue, host approvals, agent application review, payment ledger audit. |
| **Host (Couple)** | Indian Wedding Families / Couples | Host Dashboard (`/dashboard/listings`) | Listing creation, event schedule management, guest approvals, check-in monitoring, INR earnings tracking. |
...
| **Agent** | Freelance Referral Partners | Agent Portal (`/dashboard/referrals`) | Referral link generation (`WWI-AGENT-XXXX`), traveler acquisition tracking, 7% commission accrual, payout requests. |
...
### A. Host Verification & Listing Approval Queue (`/dashboard/admin/hosts`)
1. Navigate to `/dashboard/admin/hosts` or `/dashboard/admin/verifications`.
2. Inspect pending couple listings:
...
3. Click **Approve Listing & Publish**. The wedding status changes from `DRAFT` / `UNDER_REVIEW` to `PUBLISHED` and becomes searchable across the marketplace.
...
3. Upon approval:
   ...
   - Agent is granted access to `/dashboard/referrals` to track clicks, signups, conversions, and 7% commission payouts.
...
1. All booking transactions strictly follow the **Numbers.pdf Financial Model**:
   - **Host Share**: 72% of gross booking revenue.
   - **Platform Fee**: 28% gross revenue (from which 7% referral commission is carved for referring agents).
